...
const TARGET_TYPE = "game";
const CANONICAL_SOURCE = "rawg";

function printUsage() {
    console.log(`Usage: node collapseGameRawgDuplicates.js [options]

Collapses game duplicate UniqueMedia groups into the single RAWG-backed entry.
...
async function processGroup(group, options) {
    ...
    if (canonicalItems.length !== 1) {
        const reason =
            canonicalItems.length === 0
                ? "no RAWG survivor in group"
                : "multiple RAWG survivors in group";
        return {
            status: "skipped",
            groupKey,
            reason,
            details: {
                ...
                rawgCount: canonicalItems.length,
            },
    ...
    console.log(
        `${options.apply ? "[apply]" : "[dry-run]"} ${groupKey} -> keeping RAWG ${toIdString(survivor._id)} (${survivor.name})`
    );
...
async function run() {
    ...
    try {
        ...
        console.log(
            `${options.apply ? "Applying" : "Dry-run only"} collapse for ${groups.length} duplicate game group(s).`
        );
