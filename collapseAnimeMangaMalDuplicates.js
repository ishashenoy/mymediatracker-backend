...
const TARGET_TYPES = new Set(["anime", "manga"]);
const MAL_SOURCE = "mal";

function printUsage() {
    console.log(`Usage: node collapseAnimeMangaMalDuplicates.js [options]

Collapses anime/manga duplicate UniqueMedia groups into the single MAL-backed entry.
Runs in dry-run mode by default.

Options:
  --apply                         Execute writes. Without this flag, the script only reports.
  --limit <number>                Only scan the first N duplicate groups.
  --type <anime|manga>            Restrict processing to one media type.
...
function parseArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg === "--type") {
            const rawValue = readFlagValue(argv, index, "--type");
            const parsed = String(rawValue).trim().toLowerCase();
            if (!TARGET_TYPES.has(parsed)) {
                throw new Error("--type must be either anime or manga.");
            }
            options.type = parsed;
            index += 1;
            continue;
...
function buildGroupKey(type, normalizedName) {
    return `${type} | ${normalizedName}`;
}

function buildGroupPipeline(options) {
    const matchStage = {
        type: options.type || { $in: Array.from(TARGET_TYPES) },
    };
                ...
                updateOne: {
                    ...
                    update: {
                        $set: {
                            ...
                            custom_image_url: getPinnedCoverImage(userMedia, duplicate),
                        },
                    },
                },
            })),
...
async function processGroup(group, options) {
    ...
    const groupKey = buildGroupKey(type, normalizedName);
    ...
    const malItems = items.filter((item) => normalizeSource(item.source) === MAL_SOURCE);
    if (malItems.length !== 1) {
        const reason = malItems.length === 0 ? "no MAL survivor in group" : "multiple MAL survivors in group";
        return {
            status: "skipped",
            groupKey,
            reason,
            details: {
                ...
                malCount: malItems.length,
            },
            stats: emptyGroupStats(),
        };
    }

    const survivor = malItems[0];
    ...
    console.log(
        `${options.apply ? "[apply]" : "[dry-run]"} ${groupKey} -> keeping MAL ${toIdString(survivor._id)} (${survivor.name})`
    );
...
async function run() {
    ...
    try {
        ...
        console.log(
            `${options.apply ? "Applying" : "Dry-run only"} collapse for ${groups.length} duplicate group(s).`
        );
        ...
        for (const group of groups) {
            try {
                ...
                const groupKey = buildGroupKey(group._id.type, group._id.normalized_name);
